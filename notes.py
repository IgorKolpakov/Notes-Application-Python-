...
def show_notes(note: dict, message: str):
    ...
    if note:
        for i, record in note.items():
            print(f'{i:>3}.{record[0]:<30} {record[1]:<20} {
                  record[2]:<30} {record[3]:<20}')
    else:
        print(message)
    print('='*110 + '\n')
...
def change_note(note: dict, cid: int):
    ...
    note[cid] = [ch[0] if ch[0] else id, ch[1] if ch[1] else name,
                 ch[2] if ch[2] else comment, ch[3] if ch[3] else data_created]
